...
void OpticalFlowAffineKlt::TrackOneFeatureFast(const GrayImage &ref_image,
    ...
    PrecomputeJacobianAndHessian(ex_patch_, ex_patch_pixel_valid_, ex_patch_rows_, ex_patch_cols_, cur_pixel_uv, all_dx_, all_dy_, hessian);
    ...
    for (int32_t drow = - options().kPatchRowHalfSize; drow <= options().kPatchRowHalfSize; ++drow) {
        for (int32_t dcol = - options().kPatchColHalfSize; dcol <= options().kPatchColHalfSize; ++dcol) {
            ...
            if (cur_image.GetPixelValue(row_in_cur_image, col_in_cur_image, &cur_pixel_value)) {
                // If valid, compute residual by precomputed ex_patch in reference image.
