        ...
        for (uint32_t iter = 0; iter < options_.kMaxIteration; ++iter) {
            ...
            for (int32_t drow = - options_.kPatchHalfSize; drow <= options_.kPatchHalfSize; ++drow) {
                for (int32_t dcol = - options_.kPatchHalfSize; dcol <= options_.kPatchHalfSize; ++dcol) {
                    ...
                    float col_j = static_cast<float>(dcol) + cur_points[feature_id].x();
            ...
            if (std::isnan(v(0)) || std::isnan(v(1))) {
                status[feature_id] = NUM_ERROR;
                break;
            }

            cur_points[feature_id].x() += v(0);
            cur_points[feature_id].y() += v(1);

            if (cur_points[feature_id].x() < 0 || cur_points[feature_id].x() > cur_image->cols() ||
                cur_points[feature_id].y() < 0 || cur_points[feature_id].y() > cur_image->rows()) {
                status[feature_id] = OUTSIDE;
                break;
            }

            if (v.squaredNorm() < options_.kMaxConvergeStep) {
                status[feature_id] = TRACKED;
                break;
            }
